...
*** NEXT STEPS ***
- Import dataset
- Change column names (change them to match the FEC) (do this with 15-16 too)
- Lowercase column names
- Filter out states that aren't DC, MD, VA (Copy previous code)
- Change date field to date
- Change the zip code column to 5-digits
- put search terms into the str_detect
- Check FEC site to make sure the number of rows in R is similar to what the FEC shows for that year.

```{r}
#Loading 22-23
dmv_foods22_23 <- read_delim("~/Downloads/oppexp-2.txt", delim = "|", col_names = column_names)
...
# making the column names lower case
colnames(dmv_foods22_23) <- tolower(colnames(dmv_foods22_23))

#need to convert disbursement_date into a date field yyyy-mm-dd
#need to convert the zipcodes into a 5-digit field.

#filtering for DC, MD, VA
dmv_foods22_23 <- dmv_foods22_23 %>% 
filter(recipient_state == "DC"| recipient_state == "MD" | recipient_state == "VA")

#establishing a date column
dmv_foods22_23 <- dmv_foods22_23 %>%  mutate(disbursement_date=mdy(disbursement_date))
